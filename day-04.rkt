...
; Find wins per card ID
(define win-hash (make-hash (find-wins card-list)))
(define (find-wins-per-id card-ids)
  (map (λ (card-id) (cons card-id (dict-ref win-hash card-id))) card-ids))

; Find the new cards-ids on card list cl from win
(define number-of-cards (length card-list))
(define (find-new-card-ids acc win)
  (cond
    [(eq? (cdr win) 0) acc]
    [(> (+ (car win) (cdr win)) number-of-cards)
     (find-new-card-ids number-of-cards acc (cons (car win) (- (cdr win) 1)))]
    [else (find-new-card-ids
           (cons (+ (car win) (cdr win)) acc)
           (cons (car win) (- (cdr win) 1)))]))

; Find the cards for the next round
(define (find-new-cards-per-id cil)
  (flatten
   (map (λ (win) (find-new-card-ids '() win)) (find-wins-per-id cil)))) 

; Play game per id
(define (play-game-per-id acc cil)
  (cond
    [(null? cil) acc]
    [else (play-game-per-id (+ (length cil) acc) (find-new-cards-per-id cil))]))

(play-game-per-id 0 (map car card-list))
